"""UAlberta Data Insertion. Usage: python data_sources/alberta/insert_data.py"""
...
def main():
    logger.info("UAlberta Data Insertion")
    ...
    try:
        ...
        for campus in data["campuses"]:
            ...
            for p in campus["programs"]:
                ...
                r = cur.fetchone()
                if r:
                    cur.execute("UPDATE programs SET name=%s,degree_type=%s,description=%s,has_coop=%s,program_url=%s,career_outcomes=%s,last_updated=%s WHERE id=%s", (p["name"], p["degree_type"], p["description"], p["has_coop"], p["program_url"], p["career_outcomes"], now, r[0]))
                    logger.info(f"  Updated: {p['name']}")
                else:
                    cur.execute("INSERT INTO programs (university_id,name,slug,degree_type,description,has_coop,program_url,career_outcomes,source_url,last_updated) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)", (uid, p["name"], p["slug"], p["degree_type"], p["description"], p["has_coop"], p["program_url"], p["career_outcomes"], p["program_url"], now))
