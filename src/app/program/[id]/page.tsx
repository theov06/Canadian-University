...
export default function ProgramDetailPage() {
  ...
  if (!program) {
    return (
      <div className="p-8 text-center">
        ...
        <Link href="/search" className="text-violet-400 text-sm hover:underline">Back to search</Link>
      </div>
    );
  }
  ...
  return (
    <div className="p-6 lg:p-8 max-w-5xl mx-auto">
      <Link href="/search" className="text-xs text-violet-400 hover:underline mb-4 inline-block">Back to search</Link>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.1 }} className="glass-card rounded-xl p-6 mb-4">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div>
            ...
            <p className="text-lg font-bold text-violet-300">${program.tuition_yearly_international.toLocaleString()}</p>
          </div>
          <div>
          ...
          <div>
            ...
            <p className="text-lg font-bold text-cyan-400">${totalYearly.toLocaleString()}</p>
          </div>
          <div>
        ...
        {/* Cost bar */}
        <div className="mt-4 h-2 rounded-full bg-white/[0.03] overflow-hidden">
          <div className="h-full rounded-full bg-gradient-to-r from-violet-500 to-cyan-500" style={{ width: `${Math.min((program.tuition_yearly_international / 55000) * 100, 100)}%` }} />
        </div>
        <div className="flex justify-between text-[9px] text-[var(--text-muted)] mt-1">
          <span>$0</span><span>$55,000/yr</span>
        </div>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.15 }} className="glass-card rounded-xl p-6 mb-4">
        ...
        <div className="grid sm:grid-cols-3 gap-4 mb-4">
          <div className="glass rounded-lg p-3">
            <Tooltip content="International English Language Testing System (Academic)"><span className="text-[10px] text-[var(--text-muted)]">IELTS</span></Tooltip>
            <p className="text-lg font-bold">{program.ielts_overall}</p>
          </div>
          <div className="glass rounded-lg p-3">
            <Tooltip content="Test of English as a Foreign Language (iBT)"><span className="text-[10px] text-[var(--text-muted)]">TOEFL iBT</span></Tooltip>
            <p className="text-lg font-bold">{program.toefl_ibt}</p>
          </div>
          <div className="glass rounded-lg p-3">
            <Tooltip content="Minimum GPA — 0 means competitive/not published"><span className="text-[10px] text-[var(--text-muted)]">Min GPA</span></Tooltip>
            <p className="text-lg font-bold">{program.min_gpa > 0 ? program.min_gpa : "Competitive"}</p>
          </div>
        </div>
          ...
          <div className="grid sm:grid-cols-2 gap-1.5">
            {program.majors.map((m) => (
              <details key={m.name} className="group glass rounded-lg overflow-hidden">
                <summary className="px-3 py-2 text-xs cursor-pointer hover:text-violet-400 transition-colors list-none flex items-center justify-between">
                  {m.name}
      ...
      <div className="grid md:grid-cols-2 gap-4 mb-8">
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.25 }} className="glass-card rounded-xl p-6">
          ...
          <div className="flex flex-wrap gap-1.5">
            {program.career_outcomes.map((c) => (
              <span key={c} className="px-2.5 py-1 rounded-md glass text-[11px]">{c}</span>
            ))}
          </div>
        </motion.div>
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }} className="glass-card rounded-xl p-6">
          ...
          <p className={`text-sm font-semibold ${program.has_coop ? "text-emerald-400" : "text-[var(--text-muted)]"}`}>
      ...
      <div className="text-center">
        <a href={program.program_url} target="_blank" rel="noopener noreferrer" className="inline-block px-8 py-3 bg-gradient-to-r from-violet-600 to-violet-500 text-white rounded-xl text-sm font-medium hover:from-violet-500 hover:to-violet-400 transition-all">
